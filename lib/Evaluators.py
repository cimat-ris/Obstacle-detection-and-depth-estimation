...
class DepthStats(Stats):
	...
	def print_stats_spec(self):
		...
		print("{} Depth RMSE(log)/MAE: {},{}").format(self.string_id, self.rmse_log_acc / float(self.iterations),
																	self.mae_acc / float(self.iterations))
...
class DetectionStats(Stats):
	...
	def measure_performance(self, pred_obstacles, gt):
		iou, precision, recall, depth_m, depth_v, t, f, n, det_obs = compute_detection_stats(pred_obstacles, gt, iou_thresh = 0.3)
	...
	def print_stats_spec(self):
		...
		print("{} Detector Global Precision/Recall: {}, {}").format(self.string_id, 
																	 self.obstacles_detected / (self.obstacles_detected + self.false_pos_acc),
																	 self.obstacles_detected / (self.obstacles_detected + self.false_neg_acc))
	...
	def return_results(self):
		...
		errors['PrecisionG'] = self.obstacles_detected / (self.obstacles_detected + self.false_pos_acc)
		errors['RecallG'] = self.obstacles_detected / (self.obstacles_detected + self.false_neg_acc)
